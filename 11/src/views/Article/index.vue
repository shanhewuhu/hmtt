<template>
  <div class="article-container">
      ...
      <template v-else>
        ...
        <div class="article-detail" v-if="!!article.art_id">
          ...
          <van-cell class="user-info" center :border="false">
            ...
            />
            <div slot="title" class="user-name">{{ article.user_name }}</div>
    ...
    <div class="article-bottom" v-if="!isLoading && !!article.art_id">
      ...
      <!-- <ArticleLike :is_liked.sync="article.is_liked"></ArticleLike> -->
      <van-icon
        name="share"
        color="#777777"
    ...
    <van-share-sheet
      ...
      :options="options"
    />
    <!-- 在vue组件中，把一些和当前组件无关的不属于当前文档流中的东西放最 -->
    <van-popup v-model="addCommentShow" position="bottom">
      <AddComment
        ...
        "
      ></AddComment>
      <!-- 每次关闭 组件销毁 每次打开 组件重新创建 mounted就会再一次执行 -->
    </van-popup>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      // 初始化变量如果是数字 null 、字符串 null 或 '',数组  [] 对象 {}
      article: {},
      is404Error: false,
      showShare: false,
      options: [
